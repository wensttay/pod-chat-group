...
public class DialogDataNotificationDBImpl implements DialogDataNotification {
    ...
    private DropBoxDataFacade dbdf;
    private Gson gson;
    //
    ...
    public DialogDataNotificationDBImpl(DropBoxDataFacade dbdf) {
    ...
    public List<Notification> listNotificationByUser(String userId) throws RemoteException {
        try {
            String json = dbdf.getContent(fileName);
    ...
    public void commit() throws RemoteException {
        if (inTransation) {
            try {
                dbdf.updateMetada(fileName, gson.toJson(notificationsTransational));
                inTransation = false;
            } catch (FileNotFoundException ex) {
                throw new RemoteException(ex.getMessage(), ex);
            }
